...
public class Sort {
    ...
    /**
     * 希尔排序 最佳情况：T(n) = O(nlog2 n)  最坏情况：T(n) = O(nlog2 n)  平均情况：T(n) =O(nlog2n)
     *
     * @param array
     * @return
     */
    ...
    /**
     * 快速排序方法 最佳情况：T(n) = O(nlogn)   最差情况：T(n) = O(n2)   平均情况：T(n) = O(nlogn)
     *
     * @param array
     * @param start
     * @param end
    ...
    /**
     * 桶排序 T(n) = O(n+k)   最差情况：T(n) = O(n+k)   平均情况：T(n) = O(n2)
     *
     * @param array
     * @param bucketSize
     * @return
    ...
    public static void main(String[] args) {
        int[] array = {3, 7, 9, 0, 2, 1, 4, 6, 5, 8};
        log.info("原始数组：{}", array);
        log.info("插入排序：{}", insertSort(array));
        log.info("快速排序：{}", quickSort(array, 0, array.length - 1));
        log.info("冒泡排序：{}", bubbleSort(array));
        log.info("选择排序：{}", selectionSort(array));
        log.info("希尔排序：{}", hillSort(array));
        log.info("归并排序：{}", mergeSort(array));
        log.info("  堆排序：{}", heapSort(array));
        log.info("计数排序：{}", countingSort(array));
        log.info("  桶排序：{}", bucketSort(Lists.newArrayList(3, 7, 9, 0, 2, 1, 4, 6, 5, 8), array.length));
        log.info("基数排序：{}", radixSort(array));
    }
}
